<x-app-layout>
    <x-slot name="header">SaferWealth Analyst - Document Analysis</x-slot>
                    ...
                    @foreach($users as $user)
                        <a href="#" onclick="selectUser({{ $user->id }}); return false;"
                            ...
                            <div class="flex items-center gap-3">
                                <div
                                    class="w-10 h-10 rounded-full bg-gradient-to-br from-teal-400 to-emerald-600 flex items-center justify-center text-white font-bold text-sm">
                                    {{ strtoupper(substr($user->name, 0, 1)) }}
                                </div>
                                <div class="flex-1 min-w-0">
                                    <h3 class="font-semibold text-gray-900 text-sm truncate">{{ $user->name }}</h3>
                                    <p class="text-xs text-gray-500 truncate">{{ $user->email }}</p>
                                    <p class="text-xs text-gray-400 mt-1">{{ $user->documents_count }}
                                        doc{{ $user->documents_count != 1 ? 's' : '' }}</p>
                                </div>
                            </div>
                        </a>
                    @endforeach
            ...
            <div id="chatArea" class="hidden flex-1 flex flex-col bg-gray-50">
                ...
                <div class="bg-white border-b border-gray-200 p-6 shadow-sm">
                    <div class="flex justify-between items-start">
                        ...
                        <div class="text-right">
                            <span class="px-4 py-2 rounded-full bg-purple-100 text-purple-800 font-semibold text-sm">AI
                                Chat</span>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white border-t border-gray-200 p-6">
                    <form id="aiMessageForm" class="flex space-x-2">
                        ...
                        <input type="text" id="messageInput" name="message" placeholder="Ask about this document..."
                            class="flex-1 px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500"
                            required />
                        <button type="submit"
                            class="px-6 py-3 bg-gradient-to-r from-purple-600 to-emerald-600 text-white font-semibold rounded-lg hover:shadow-lg transition duration-300">
                            Send
                        </button>
                    </form>
                </div>
    ...
    <script>
        ...
        function backToUsers() {
            ...
            document.querySelectorAll('.user-item').forEach(el => el.classList.remove('bg-teal-50', 'border-l-4', 'border-teal-500'));
            currentUserId = null;
            currentDocId = null;
        }
        ...
        function selectUser(userId) {
            ...
            document.querySelectorAll('.user-item').forEach(el => el.classList.remove('bg-teal-50', 'border-l-4', 'border-teal-500'));
            document.querySelector(`[data-user-id="${userId}"]`).classList.add('bg-teal-50', 'border-l-4', 'border-teal-500');
        ...
        function loadDocument(docId) {
            ...
            document.querySelectorAll('.document-item').forEach(el => el.classList.remove('bg-teal-50', 'border-l-4', 'border-teal-500'));
            document.querySelector(`[data-doc-id="${docId}"]`).classList.add('bg-teal-50', 'border-l-4', 'border-teal-500');
            ...
            fetch(`/admin/conversations/${convId}/messages/${msgId}`, {
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        const docId = document.querySelector('[data-doc-id].bg-teal-50')?.getAttribute('data-doc-id');
